(function ($) {
	$.fn.rainbowText = function (options) {
		...
		for (var i = 0; i < this.options.data.length; i++) {
			this.options.el.append("<span>" + this.options.data.charAt(i) + "</span>");
		}
		setInterval(this.moveRainbow);
	}
	$.fn.rainbowText.prototype.moveRainbow = function () {
		...
		for (var i = 0; i < a; i++) {
			...
			h += this.options.hInc;
			this.el.children().eq(i).css("color", "rgb(" + red + ", " + grn + ", " + blu + ")");
		}
